#!/usr/bin/env bash
# Bench harness for the I/O-bound workload (sleep-based downstream simulation).
#
# Usage: ./poc_bench_io_bound.sh <variant> [run_idx]
#   variant: ractor_pool | cluster | cluster_nate | single_threaded
#   run_idx: optional integer, used to suffix output files (default 1)
#
# Tunable: IO_WAIT_MS env var sets the per-request sleep (default 20ms).
#
# All variants serve poc_io_bound_app.ru on 127.0.0.1:9292. The point of
# this bench is the falsifier: when the GVL is released during the I/O
# wait, ractor_pool and single_threaded should converge — proving the
# realistic-bench RPS gap is GVL-specific, not "Ractors are faster."
...
for i in $(seq 1 100); do
  if curl -s -o /dev/null -m 2 "$url"; then booted=1; break; fi
  sleep 0.2
done
if [ -z "${booted:-}" ]; then
...
# Lightweight shape check — io_bound app returns {ok, waited_ms, path}.
shape=$(curl -s "$url")
if ! printf '%s' "$shape" | grep -q '"ok":true'; then
  echo "[$tag] ERROR: unexpected response shape: $shape" >&2
  kill -9 "$pid" 2>/dev/null || true
  exit 4
fi
echo "[$tag] verify: $shape"
